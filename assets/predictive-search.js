...
class PredictiveSearchComponent extends Component {
  ...
  connectedCallback() {
    ...
    if (categorySelect) {
      categorySelect.addEventListener('change', (e) => this.onCategoryChange(e), { signal });
    }
  ...
  #filterDomByCategory(categoryHandle) {
    if (!categoryHandle) return;
    const { predictiveSearchResults } = this.refs;
    if (!predictiveSearchResults) return;

    const cards = predictiveSearchResults.querySelectorAll('.predictive-search-results__card--product');
    let visibleCount = 0;

    cards.forEach((card) => {
      const collections = card.dataset.productCollections || '';
      if (!collections || collections.includes(categoryHandle)) {
        card.style.display = '';
        visibleCount++;
      } else {
    ...
    if (cards.length > 0 && visibleCount === 0) {
      let noResults = predictiveSearchResults.querySelector('.predictive-search-results__no-results');
      if (!noResults) {
        const noResultsP = document.createElement('p');
        noResultsP.className = 'predictive-search-results__no-results';
        noResultsP.textContent = 'لا توجد نتائج تطابق هذا التصنيف';
        predictiveSearchResults.querySelector('.predictive-search-results__inner')?.appendChild(noResultsP);
      }
    }
  }
  ...
  async #getSearchResults(searchTerm) {
    ...
    sectionRenderer
      ...
      .then((resultsMarkup) => {
        ...
        this.#filterDomByCategory(categoryHandle);
